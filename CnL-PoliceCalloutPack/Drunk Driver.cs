...
{
    [CalloutProperties("DUI", "LosAngelesi", "1.1")]
    public class DUI : FivePD.API.Callout
    {
        ...
        public async override void OnStart(Ped player)
        {
            ...
            API.TaskVehicleDriveWander(driver.Handle, vehicle.Handle, 35f, (int)DrivingStyle.Rushed);
            ...
            if (chance >= 0 && chance <= 0.5)
            {
                ...
                pursuit = Pursuit.RegisterPursuit(driver);
                if (pursuit != null)
                {
            ...
            // The suspect is slumped over in a stopped vehicle
            int slumpedChance = rnd.Next(0, 100);
            if (slumpedChance <= 10)
            {
                await driver.Task.PlayAnimation("amb@world_human_aa_smoke@male@idle_a", "idle_a", 8f, -1, -1, (AnimationFlags)49, 0);
            }

            int animationChance = rnd.Next(0, 100);
            if (animationChance <= 50)
            {
                await driver.Task.PlayAnimation("move_m@drunk@verydrunk", "idle", 8f, -1, -1, (AnimationFlags)49, 0);
            }
            else
            {
                await driver.Task.PlayAnimation("move_m@drunk@moderatedrunk", "idle", 8f, -1, -1, (AnimationFlags)49, 0);
            }
        }
